...
//    navigator.camera.getPicture(onSuccess, onFail, {quality:80, destinationType:Camera.DestinationType.DATA_URL});
    navigator.camera.getPicture(onSuccess, onFail, {quality:80, destinationType:Camera.DestinationType.FILE_URI});
   ...
   //     image.src = "data:image/jpeg;base64," + imageData;
    ...
    function onFail(message) {
        navigator.notification.alert("촬영중 오류가 발생했습니다",alertDismissed,"Nnanyo"," 확 인 ");
    }
}
...
function getPhoto() {
   navigator.camera.getPicture(getPhotoOnSuccess, onFail, {quality:80, destinationType:Camera.DestinationType.FILE_URI, sourceType:navigator.camera.PictureSourceType.SAVEDPHOTOALBUM});
}

function onFail (message) {
    navigator.notification.alert("앨범을 가져오는 중 오류가 발생했습니다",alertDismissed,"Nnanyo"," 확 인 ");
}
...
function fail(error) {
    ...
    navigator.notification.alert("앨범을 업로드 하는 중 오류가 발생했습니다",alertDismissed,"Nnanyo"," 확 인 ");
...
//                        document.getElementById('picture_msg').innerHTML = "Upload failed: Code = "+error.code;
  //                  }, options);
    ...
    $("#backBtn").on("click",function() {
        navigator.notification.confirm("입력을 취소하시겠습니까?", confirmCallBack, "Nnanyo", " 취소 , 취소 안함 ");
//        location.href="index.html";
    }) 
